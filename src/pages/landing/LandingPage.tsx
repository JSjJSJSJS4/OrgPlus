...
export const LandingPage: React.FC<LandingPageProps> = ({ setCurrentTab, userRole }) => {
  ...
  return (
    <div className="space-y-20 pb-10">
      ...
      <section className="relative rounded-3xl overflow-hidden min-h-[500px] flex items-center p-8 sm:p-12 md:p-16 border border-white/5 bg-gradient-premium">
        ...
        <div className="relative max-w-2xl space-y-6 text-left">
          ...
          <p className="text-slate-350 text-xs sm:text-sm leading-relaxed max-w-lg">
          ...
          <div className="pt-4 flex flex-wrap gap-3.5">
            ...
            <a 
              href="#about"
              className="px-6 py-3 bg-slate-900 border border-slate-800 hover:bg-slate-850 text-xs font-bold text-slate-300 rounded-xl cursor-pointer transition-colors inline-block"
            >
              Learn More
            </a>
          </div>
      ...
      {activeCampaigns.length > 0 && (
        <section className="space-y-6">
          <div className="text-center max-w-lg mx-auto space-y-2">
            <span className="text-[10px] font-black text-purple-400 uppercase tracking-widest block">Recruitment Spotlight</span>
            <h2 className="text-2xl font-black text-white">Join Our Open Positions</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {activeCampaigns.map(c => (
              <div 
                key={c.id} 
                className="p-6 rounded-2xl glass-panel border border-purple-500/10 hover:border-purple-500/20 relative overflow-hidden flex flex-col justify-between"
              >
                <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/5 rounded-full blur-xl pointer-events-none" />
                <div className="text-left">
                  <div className="flex justify-between items-center">
                    <span className="px-2.5 py-0.5 rounded-lg text-[9px] font-extrabold uppercase bg-purple-950/40 border border-purple-500/20 text-purple-400">
                      {c.type} recruitment
                    </span>
                    <span className="text-[10px] font-semibold text-slate-500">Active Campaign</span>
                  </div>
                  <h3 className="text-base font-bold text-white mt-4">{c.title}</h3>
                  <p className="text-xs text-slate-450 mt-2 leading-relaxed line-clamp-3">{c.description}</p>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-900/60 flex justify-between items-center">
                  ...
                  <button
                    onClick={() => setCurrentTab('recruitment')}
                    className="px-4 py-2 bg-purple-600/20 hover:bg-purple-600 text-purple-300 hover:text-white border border-purple-500/20 hover:border-transparent text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1"
                  >
      ...
      <section id="about" className="grid md:grid-cols-3 gap-6 text-left">
        <div className="p-6 rounded-2xl glass-panel border border-slate-800/80 flex flex-col justify-between space-y-8">
          <div>
            ...
            <h3 className="text-base font-extrabold text-white mt-4">Our Core Mission</h3>
          ...
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Mission Statement</span>
        </div>
        ...
        <div className="p-6 rounded-2xl glass-panel border border-slate-800/80 flex flex-col justify-between space-y-8">
          <div>
            ...
            <h3 className="text-base font-extrabold text-white mt-4">Our Future Vision</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          ...
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Vision Statement</span>
        </div>
        ...
        <div className="p-6 rounded-2xl glass-panel border border-slate-800/80 flex flex-col justify-between space-y-8">
          <div>
            ...
            <h3 className="text-base font-extrabold text-white mt-4">Pillars of Excellence</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          ...
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Organizational Pillars</span>
        </div>
      </section>
      ...
      <section className="space-y-6">
        <div className="text-center max-w-lg mx-auto space-y-2">
          <span className="text-[10px] font-black text-purple-400 uppercase tracking-widest block">Executive Board</span>
          <h2 className="text-2xl font-black text-white">Meet Our Leaders</h2>
          <p className="text-xs text-slate-400 leading-relaxed">
        ...
        <div className="grid sm:grid-cols-3 gap-6 text-left">
          {officers.map(off => (
            <div key={off.name} className="p-6 rounded-2xl glass-panel border border-slate-850 flex flex-col items-center text-center space-y-4 relative">
              ...
              <div>
                <h4 className="text-sm font-bold text-white leading-tight">{off.name}</h4>
                <p className="text-[9px] font-extrabold uppercase text-purple-400 tracking-wider mt-1">{off.title}</p>
              </div>
              <p className="text-xs text-slate-400 leading-relaxed italic border-t border-slate-900/60 pt-3">
                "{off.quote}"
              </p>
            </div>
          ))}
      ...
      <section className="space-y-6">
        <div className="text-center max-w-lg mx-auto space-y-2">
          <span className="text-[10px] font-black text-purple-400 uppercase tracking-widest block">Snapshots</span>
          <h2 className="text-2xl font-black text-white">Featured Org Activities</h2>
          <p className="text-xs text-slate-400 leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {galleryImages.map((img, i) => (
            <div 
              key={i} 
              className="group rounded-2xl overflow-hidden aspect-[4/3] border border-slate-800 relative cursor-pointer"
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-4 text-left bg-gradient-to-t from-slate-950/90 to-transparent">
                ...
                <p className="text-[10px] text-slate-400 mt-0.5 truncate">{img.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {featuredEvents.length > 0 && (
        <section className="space-y-6">
          <div className="text-center max-w-lg mx-auto space-y-2">
            <span className="text-[10px] font-black text-purple-400 uppercase tracking-widest block">Events Feed</span>
            <h2 className="text-2xl font-black text-white">Featured Announcements</h2>
            <p className="text-xs text-slate-400 leading-relaxed">
          ...
          <div className="grid md:grid-cols-3 gap-6 text-left">
            {featuredEvents.map(ev => (
              <div key={ev.id} className="p-6 rounded-2xl glass-panel border border-slate-800/80 flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="px-2 py-0.5 rounded-lg text-[9px] font-extrabold uppercase bg-purple-950/25 border border-purple-500/20 text-purple-400">
                      {ev.status}
                    </span>
                  </div>
