...
export const RecruitmentPage: React.FC = () => {
  ...
  const renderStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400">Approved</span>
      case 'rejected':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400">Rejected</span>
      case 'screening':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 text-blue-700 dark:text-blue-400">In Screening</span>
      case 'interview':
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/30 text-purple-700 dark:text-purple-400">Interviewing</span>
      default:
        return <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-amber-700 dark:text-amber-400">Pending Review</span>
    }
  }

  const renderCampaignStatusBadge = (status: CampaignStatus) => {
    if (status === 'active') {
      return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400">Active</span>
    }
    if (status === 'draft') {
      return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400">Draft</span>
    }
    return <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400">Closed</span>
  }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Loading campaigns...</p>
        </div>
      ) : activeTab === 'campaigns' ? (
        <div className="grid md:grid-cols-2 gap-6">
          {campaigns.length === 0 ? (
            <div className="md:col-span-2 glass-panel border border-slate-200 dark:border-slate-800/80 p-12 text-center rounded-2xl">
              <Users className="w-12 h-12 text-slate-400 dark:text-slate-500 mx-auto mb-4" />
              <p className="text-slate-700 dark:text-slate-300 font-bold text-sm">No Active Campaigns Available</p>
              <p className="text-slate-500 text-xs mt-1">Check back later or ask your organization officer.</p>
            </div>
          ) : (
            campaigns.map(campaign => (
              <div key={campaign.id} className="p-6 rounded-2xl glass-panel border border-slate-200 dark:border-slate-800/80 glass-panel-hover flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start gap-4">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wide border ${
                      campaign.type === 'officer' 
                        ? 'border-purple-250 bg-purple-50 text-purple-700 dark:border-purple-500/30 dark:bg-purple-950/40 dark:text-purple-400' 
                        : 'border-blue-250 bg-blue-50 text-blue-700 dark:border-blue-500/30 dark:bg-blue-950/40 dark:text-blue-400'
                    }`}>
                      {campaign.type} recruitment
                    </span>
                    {renderCampaignStatusBadge(campaign.status)}
                  </div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white mt-3.5 leading-tight">{campaign.title}</h3>
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed line-clamp-3">{campaign.description}</p>
                </div>

                <div className="mt-6 pt-4 border-t border-slate-200/60 dark:border-slate-900/60 flex justify-between items-center">
        ...
        <div className="space-y-6">
          {myApps.length === 0 ? (
            <div className="glass-panel border border-slate-200 dark:border-slate-800/80 p-12 text-center rounded-2xl">
              <FileText className="w-12 h-12 text-slate-400 dark:text-slate-500 mx-auto mb-4" />
              <p className="text-slate-700 dark:text-slate-300 font-bold text-sm">You haven't submitted any applications yet.</p>
              <p className="text-slate-500 text-xs mt-1">Browse active recruitment campaigns above to get started!</p>
            </div>
          ) : (
            myApps.map(app => (
              <div key={app.id} className="p-6 rounded-2xl glass-panel border border-slate-200 dark:border-slate-800/80 relative overflow-hidden">
                {/* Visual Progress Stepper */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200/60 dark:border-slate-900/60 pb-6 mb-6">
                  <div>
                    <h3 className="text-base font-bold text-slate-900 dark:text-white leading-tight">
                      {app.recruitment_campaigns?.title || 'Unknown Drive'}
                    </h3>
                    <p className="text-[10px] text-slate-550 dark:text-slate-400 mt-1">Submitted on {new Date(app.created_at).toLocaleDateString()}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Application Status:</span>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 dark:text-slate-350 uppercase tracking-wide">Questionnaire Responses</h4>
                    {Object.entries(app.answers).map(([question, answer]) => (
                      <div key={question} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/20 border border-slate-200 dark:border-slate-900">
                        <p className="text-[11px] font-bold text-purple-700 dark:text-purple-300">{question}</p>
                        <p className="text-xs text-slate-705 dark:text-slate-300 mt-1 leading-relaxed">{answer}</p>
                      </div>
                    ))}
                  </div>

                  {/* Actions / Next steps box */}
                  <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/80 flex flex-col justify-between">
                    <div>
                      <h4 className="text-xs font-bold text-slate-500 dark:text-slate-300 uppercase tracking-wide mb-3">Next Steps</h4>
                      {app.status === 'pending' && (
                      ...
                      {app.status === 'screening' && (
                        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed text-blue-700 dark:text-blue-300">
                          Officers are reviewing your responses and credentials. We may contact you for details.
                        </p>
                      )}
                      {app.status === 'interview' && (
                        <div className="space-y-2">
                          <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed text-purple-750 dark:text-purple-300">
                            Congratulations! You've advanced to the interview round.
                          </p>
                          {app.feedback && (
                            <div className="p-2 bg-purple-50 dark:bg-purple-950/20 rounded border border-purple-200 dark:border-purple-900 text-[10px] text-purple-700 dark:text-purple-300">
                      ...
                      {app.status === 'rejected' && (
                        <div>
                          ...
                          {app.feedback && (
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-2 bg-slate-100 dark:bg-slate-950/30 p-2 rounded">
                    ...
                    {app.status === 'approved' && (
                      <div className="mt-4">
                        {app.onboarding_status === 'completed' ? (
                          <div className="flex items-center gap-2 text-xs font-bold text-emerald-750 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 p-2.5 rounded-xl border border-emerald-200 dark:border-emerald-900/30">
                            <CheckCircle className="w-4 h-4 text-emerald-500" /> Onboarding Completed
                          </div>
                        ) : (
                          <button
                            onClick={() => {
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-lg glass-panel border border-slate-200 dark:border-slate-800/80 p-6 rounded-2xl shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Launch Recruitment Drive</h3>
            <form onSubmit={handleCreateCampaign} className="space-y-4">
              <div>
                <label className="block text-xs text-slate-550 dark:text-slate-400 font-semibold mb-1">Campaign Title</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs text-slate-550 dark:text-slate-400 font-semibold mb-1">Description & Requirements</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-slate-550 dark:text-slate-400 font-semibold mb-1">Recruitment Target</label>
                  <select
                ...
                <div>
                  <label className="block text-xs text-slate-550 dark:text-slate-400 font-semibold mb-1">Closes Date</label>
                  <input
                    type="date"
              ...
              >
                <Send className="w-3.5 h-3.5 text-white" /> Launch Recruitment Drive
              </button>
            </form>
          </div>
        </div>
      ...
      {selectedCampaign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-xl glass-panel border border-slate-200 dark:border-slate-800/80 p-6 rounded-2xl shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-purple-700 dark:text-purple-400 font-bold mb-4">{selectedCampaign.title}</p>
            ...
            <form onSubmit={handleApplySubmit} className="space-y-4">
              ...
              >
                {submitting ? (
                  ...
                  <>
                    <Send className="w-3.5 h-3.5 text-white" /> Submit Application
                  </>
                )}
              </button>
            </form>
